...
def main(argv):
    ...
    parser.add_argument('-r', '--rounds', default=MAX_FL_ROUNDS, type=int,
    ...
    parser.add_argument('-t', '--training_mode', default='flad', type=str, nargs='?', choices=['flad', 'flddos', 'fedavg_1e', 'fedavg_5e'],
                        help='Federated training mode. Available options are: flad, flddos, fedavg_1e, fedavg_5e')
    ...
    if args.clients is not None:
        ...
        if args.training_mode is not None:
            if args.training_mode == "flad":
                FederatedTrain(clients, args.model, output_folder, time_window, max_flow_len, dataset_name,
                               epochs=epochs, steps=steps, training_mode=args.training_mode, max_rounds = args.rounds,weighted=False,
                               optimizer=args.optimizer, nr_experiments=EXPERIMENTS)
            # FedAvg setup: rundom subset of clients and 1 epoch of local training per round (steps=0 means 1/batch_size)
            elif args.training_mode == "fedavg_1e":
                FederatedTrain(clients, args.model, output_folder, time_window, max_flow_len, dataset_name,
                               epochs=1, steps=0, training_mode=args.training_mode, max_rounds = args.rounds, weighted=True,
                               optimizer=args.optimizer, nr_experiments=EXPERIMENTS)
            # FedAvg setup: rundom subset of clients and 5 epochs of local training per round (steps=0 means 1/batch_size)
            elif args.training_mode == "fedavg_5e":
                FederatedTrain(clients, args.model, output_folder, time_window, max_flow_len, dataset_name,
                               epochs=5, steps=0, training_mode=args.training_mode, max_rounds = args.rounds, weighted=True,
                               optimizer=args.optimizer, nr_experiments=EXPERIMENTS)
            # FLDDoS setup: rundom subset of clients and 10 epochs of local training per round + local model
            elif args.training_mode == "flddos":
                FederatedTrain(clients, args.model, output_folder, time_window, max_flow_len, dataset_name,
                               epochs=FLDDOS_EPOCHS, steps=0, training_mode=args.training_mode, max_rounds = args.rounds, weighted=True,
                               optimizer=args.optimizer, nr_experiments=EXPERIMENTS)
